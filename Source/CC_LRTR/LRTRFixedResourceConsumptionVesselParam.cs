...
namespace ContractConfigurator.LRTR
{
    ...
    public class LRTRFixedResourceConsumptionChecker : MonoBehaviour
    {
        ...
        public static LRTRFixedResourceConsumptionChecker Instance;
    ...
    /// </summary>
    public class LRTRFixedResourceConsumption : VesselParameter
    {
        ...
        public LRTRFixedResourceConsumption()
            : base(null)
        {
        }

        public LRTRFixedResourceConsumption(double minRate, double maxRate, PartResourceDefinition resource, string title = null)
            : base(title)
        {
        ...
        protected override string GetParameterTitle()
        {
            ...
            if (string.IsNullOrEmpty(title))
            {
                ...
                {
                    resourceStr = Localizer.Format("#cc.param.LRTRFixedResourceConsumption.atLeast", minRate.ToString("N1"));
                }
                else if (minRate == 0.0)
                {
                    resourceStr = Localizer.Format("#cc.param.LRTRFixedResourceConsumption.atMost", maxRate.ToString("N1"));
                }
                else if (minRate == double.MinValue)
                {
                    resourceStr = Localizer.Format("#cc.param.LRTRFixedResourceConsumption.atLeast", (-maxRate).ToString("N1"));
                }
                else if (maxRate == 0.0)
                {
                    resourceStr = Localizer.Format("#cc.param.LRTRFixedResourceConsumption.atMost", (-minRate).ToString("N1"));
                }
                else if (minRate >= 0)
                {
                    resourceStr = Localizer.Format("#cc.param.LRTRFixedResourceConsumption.between", minRate.ToString("N1"), maxRate.ToString("N1"));
                }
                else if (minRate < 0)
                {
                    resourceStr = Localizer.Format("#cc.param.LRTRFixedResourceConsumption.between", (-maxRate).ToString("N1"), (-minRate).ToString("N1"));
                }
                else
                {
                    // Shouldn't happen
                ...
                output = Localizer.Format((minRate >= 0.0 && maxRate > 0.0 ? "#cc.param.LRTRFixedResourceConsumption.consumption" : "#cc.param.LRTRFixedResourceConsumption.production"), resource.name, resourceStr);
            }
            else
            {
                output = title;
        ...
        {
            LRTRFixedResourceConsumptionChecker.Register(resource);
            base.OnRegister();
        }

        protected override void OnUnregister()
        {
            LRTRFixedResourceConsumptionChecker.UnRegister();
            base.OnUnregister();
        }
        ...
        protected override bool VesselMeetsCondition(Vessel vessel)
        {
            ...
            double delta = 0.0;
            if (LRTRFixedResourceConsumptionChecker.CanCheckVessel(vessel))
            {
                delta = LRTRFixedResourceConsumptionChecker.Instance.Consumption(resource);
            }
            else
            {
                return false;
