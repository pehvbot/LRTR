...
namespace KerbalConstructionTime
{
    public static partial class KCT_GUI
    {
        ...
        public static void InitBuildListVars()
        {
            ...
            _settingsTexture = new GUIContent(GameDatabase.Instance.GetTexture("LRTR/KCT/resources/KCT_settings16", false));
            _planeTexture = new GUIContent(GameDatabase.Instance.GetTexture("LRTR/KCT/resources/KCT_flight16", false));
            _rocketTexture = new GUIContent(GameDatabase.Instance.GetTexture("LRTR/KCT/resources/KCT_rocket16", false));
        }
        ...
        public static void DrawBuildListWindow(int windowID)
        {
            ...
            if (buildItem != null)
            {
               ...
               if(buildItem.GetTimeLeft() > 30)
                {
                    if (KCTGameStates.Settings.AutoKACAlarms && KACWrapper.APIReady) //don't check if less than 30 seconds to completion. Might fix errors people are seeing
                    {
                        DoKACAlarms(buildItem);
                    }
                    if (KCTGameStates.Settings.AutoStockAlarms) //don't check if less than 30 seconds to completion. Might fix errors people are seeing
                    {
        ...
        static void DoStockAlarms(IKCTBuildItem buildItem)
        {
            ...
            if (!Utilities.IsApproximatelyEqual(KCTGameStates.StockAlarmUT - UT, buildItem.GetTimeLeft()))
            {
                KCTDebug.Log("Stock Alarm being created!");
                KCTGameStates.StockAlarmUT = (buildItem.GetTimeLeft() + UT);

                bool alarm = AlarmClockScenario.AlarmExists(KCTGameStates.StockAlarmId);
                if (alarm)
                {
                    KCTDebug.Log("Removing existing alarm");
                    AlarmClockScenario.DeleteAlarm(KCTGameStates.StockAlarmId);
                }
                ...
                AlarmTypeRaw alarmToSet = new AlarmTypeRaw
                {
                    ...
                    ut = KCTGameStates.StockAlarmUT
                };
                AlarmClockScenario.AddAlarm(alarmToSet);
                KCTGameStates.StockAlarmId = alarmToSet.Id;
